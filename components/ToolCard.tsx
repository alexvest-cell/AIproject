    ...
    if (compact) {
        return (
            <div className="bg-zinc-900/50 backdrop-blur-sm border border-white/10 rounded-xl p-4 flex items-center gap-4 hover:border-news-accent hover:shadow-[0_0_15px_rgba(43,212,195,0.15)] transition-all group">
                {tool.logo ? (
                    <Image src={tool.logo} alt={tool.name} width={48} height={48} className="w-12 h-12 rounded-lg object-contain bg-white p-1" unoptimized />
                ) : (
            ...
            <div className="p-6 md:p-8">
                <div className="flex flex-col md:flex-row gap-6 items-start">
                    ...
                    <div className="flex flex-col items-center gap-4 w-full md:w-auto">
                        {tool.logo ? (
                            <Image src={tool.logo} alt={tool.name} width={96} height={96} className="w-20 h-20 md:w-24 md:h-24 rounded-2xl object-contain bg-white p-3 shadow-inner" unoptimized />
                        ) : (
